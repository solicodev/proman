        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table id="table" class="table table-striped table-bordered">
                    ...
                    <tbody>
                    @foreach ($campaignBrands as $campaignBrand)
                        <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td><span class="badge bg-{{$campaignBrand->color ?? null}} ">{{ $campaignBrand->name }}</span></td>
                            <td>
                                <div class="d-flex">
                                    <a href="#"
                                       onclick="openEditModal('{{ route('admin.campaign.brand.update', $campaignBrand->id) }}', JSON.stringify({name:'{{ $campaignBrand->name }}' , parent_id:'{{$campaignBrand->parent_id}}'}))"
                                       class='text-warning'>
                                        <i class="bx bxs-edit"></i>
                                    </a>
                                    <a href="#"
                                       onclick="openDeleteModal('{{ route('admin.campaign.brand.destroy', $campaignBrand->id) }}')"
    ...
    <div class="modal fade" id="createCategoryModal" tabindex="-1" aria-labelledby="createCategoryModalLabel"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.campaign.brand.store') }}" method="post" id='createForm'>
                    @csrf
