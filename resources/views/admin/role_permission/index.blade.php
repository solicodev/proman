            ...
            <div class="col-lg-12">
                <div class="card">
                            ...
                            <table class="table table-bordered text-nowrap border-bottom w-100" id="responsive-datatable">
                                ...
                                @foreach($roles as $role)
                                    <tr>
                                        ...
                                        <td>{{ $role->name }}</td>
                                        <td>
                                            <a href="{{ route('role-permission.edit', $role->id) }}" class="btn btn-primary fs-14 text-white edit-icn" title="ویرایش">
                                            ...
                                            <form id="form-{{ $role->id }}" action="{{ route('role-permission.destroy', $role->id) }}" method="post">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </td>
                    ...
                    <div class="card-footer">
                        <a href="{{ route('role-permission.create') }}" class="btn btn-primary">افزودن نقش</a>
                    </div>
                </div>
            </div>
        </div>
